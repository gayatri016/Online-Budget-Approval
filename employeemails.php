...
<a href="employeemails.php" class="btn btn-info btn-sm"><i class ="fa fa-angle-double-left"></i> &nbsp; Back</a>
<div class="row">
...
<br><br>
 ...
 <div class="log">
<form method="POST" action="employeecomposeaction.php" enctype="multipart/form-data" >
...
while ($row=mysqli_fetch_array($sql)) { ?>
  ...
  <option value=<?php echo $row['emailid'];?> style="color:#6600ff"><?php echo $row['fullname'] ." (". $row['emailid'] ;?></option>
  <?php } ?>
  <option value=""> Please Select Manager </option> 
 <?php     
include"connection.php";
$sql1=mysqli_query($db,"SELECT DISTINCT a.deptid FROM register r, assignmanagers a WHERE r.emailid=a.mailid AND r.emailid='$email' ");
while ($row1=mysqli_fetch_array($sql1)) {
$did=$row1[0];
}
$sql3=mysqli_query($db,"SELECT r.fullname, r.emailid, d.deptname, d.deptlocation FROM register r, assignmanagers a, department d WHERE r.emailid=a.mailid AND a.deptid=d.id AND r.role='manager' AND a.deptid='$did' ");
   
while ($row3=mysqli_fetch_array($sql3)) { ?>

  <option value=<?php echo $row3['emailid'];?> style="color:#cc0000"><?php echo $row3['fullname'] ." (". $row3['emailid'] .") - ". $row3['deptname'] ." (". $row3['deptlocation'] .")" ;?></option>
  <?php } ?> 
</select>
</div>
</div>
...
<td>
    <a href="employeereply.php?id=<?php echo  $row['id'] ?>" class="btn btn-warning btn-sm">Reply</a>
</td>
